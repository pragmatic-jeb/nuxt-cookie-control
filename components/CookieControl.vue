  ...
  <client-only>
    <section class="cookieControl" v-if="cookies.text">
      <transition :name="`cookieControl__Bar--${cookies.barPosition}`">
        <div :class="`cookieControl__Bar cookieControl__Bar--${cookies.barPosition}`" v-if="colorsSet && showCookieBar()">
          <div class="cookieControl__BarContainer">
            ...
            <div class="cookieControl__BarButtons">
              <button @click="setConsent({declineAll: true, consent: false, reload:true})" v-text="cookies.text.declineAll"/>
              <button @click="setConsent({reload: true, consent: true})" v-text="cookies.text.acceptAll"/>
            </div>

            <div class="cookieControl__BarFooter"><p>View `use of tracking features` section in <a href="/privacy-policy/">privacy notice</a></p></div>
...
export default {
  ...
  methods: {
    ...
    setConsent({type='full', consent=true, reload=true, declineAll=false}){
      ...
      this.cookies.set({domain: this.$cookies.domain, name: 'cookie_control_enabled_cookies', value: consent ? enabledCookies.join(',') : '', expires: this.expirationDate});
